...
SituationAssessor::SituationAssessor(const std::string& agent_name, bool is_robot) : facts_publisher_(&n_, agent_name), facts_calculator_(&n_)
{
  ...
  ***************************/
  ...
  auto panda_joint_perception =  new RobotJointsPerception(&n_, "dual_pandas",  // TODO how to avoid fix name ? 
                                                         {{"panda_1_link8", owds::BODY_PART_LEFT_HAND},{"panda_2_link8", owds::BODY_PART_RIGHT_HAND}},
                                                         bullet_client_, 0.09);
  robots_manager_.addPerceptionModule("panda_joints", panda_joint_perception);
