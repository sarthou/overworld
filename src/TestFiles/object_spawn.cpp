...
int main(int argc, char** argv)
{
  ...
  */
  ...
  /*
  overworld::EntitiesPoses entities;
  entities.entities = {createObject(3.0, 2.0, 1.75, "p1", "Pen")};
  overworld::EntitiesPoses false_entities;
  false_entities.entities = {createObject(4.0, 2.0, 1.75, "f1", ""),createObject(1.0, 2.0, 1.75, "f2", "")};
  pub.publish(entities);
  ros::spinOnce();
  r.sleep();
  ...
  for(int i = 0; i < 200; i++)
  {
    ...
    false_pub.publish(false_entities);
    false_entities.entities[0].pose.pose.position.x -= 0.01;
    false_entities.entities[1].pose.pose.position.x += 0.01;
    ros::spinOnce();
    r.sleep();
  }
  return 0;
  */
  ...
  /*
  overworld::EntitiesPoses entities;
  entities.entities = {createObject(2.0, 2.0, 1.75, "p1", "Pen")};
  overworld::EntitiesPoses false_entities;
  false_entities.entities = {createObject(3.0, 2.0, 1.75, "f1", "")};
  pub.publish(entities);
  ros::spinOnce();
  ...
  */
  ...
  overworld::EntitiesPoses entities;
  entities.entities = {createObject(2.0, 2.0, 1.75, "p1", "Head")};
  overworld::EntitiesPoses false_entities;
  false_entities.entities = {createObject(3.0, 2.0, 1.75, "f1", "")};
  pub.publish(entities);
  ros::spinOnce();
