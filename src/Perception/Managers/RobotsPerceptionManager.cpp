...
Agent* RobotsPerceptionManager::getAgent(const std::string& agent_name)
{
  ...
  if(it == agents_.end())
  {
    auto agent = new Agent(agent_name, FieldOfView(34.8, 64.1,0.1,5), AgentType_e::ROBOT);
...
void RobotsPerceptionManager::UpdateAgent(BodyPart* body_part)
{
  if(body_part->isAgentKnown())
  {
    ...
    {
      auto agent = new Agent(body_part->getAgentName(), FieldOfView(54.8, 84.1,0.1,5), AgentType_e::ROBOT);
