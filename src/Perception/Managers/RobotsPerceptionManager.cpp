...
Agent* RobotsPerceptionManager::getAgent(const std::string& agent_name)
{
  ...
  if(it == agents_.end())
  {
    auto agent = new Agent(agent_name, FieldOfView(34.8, 64.1,0.1,12), AgentType_e::ROBOT);
...
void RobotsPerceptionManager::UpdateAgent(BodyPart* body_part)
{
  if(body_part->isAgentKnown())
  {
    ...
    {
      auto agent = new Agent(body_part->getAgentName(), FieldOfView(54.8, 84.1,0.1,12), AgentType_e::ROBOT);
    ...
    {    
      case BODY_PART_HEAD:
      {
        ...
        ShellDisplay::info("[RobotsPerceptionManager] Head has been setted for " + it_agent->second->getId());
        break;
      }
      case BODY_PART_LEFT_HAND:
      {
        it_agent->second->setLeftHand(static_cast<Hand*>(body_part));
        ShellDisplay::info("[RobotsPerceptionManager] Left hand has been setted for " + it_agent->second->getId());
        break;
      }
      case BODY_PART_RIGHT_HAND:
      {
        it_agent->second->setRightHand(static_cast<Hand*>(body_part));
        ShellDisplay::info("[RobotsPerceptionManager] Right hand has been setted for " + it_agent->second->getId());
        break;
      }
      case BODY_PART_TORSO:
      {
        it_agent->second->setTorso(body_part);
        ShellDisplay::info("[RobotsPerceptionManager] Torso has been setted for " + it_agent->second->getId());
        break;
      }
      case BODY_PART_BASE:
      {
        it_agent->second->setBase(body_part);
        ShellDisplay::info("[RobotsPerceptionManager] Base has been setted for " + it_agent->second->getId());
        break;
      }
    }
  }
  else
    ShellDisplay::warning("[RobotsPerceptionManager] The agent of the body part " + body_part->id() + " is undefined");
}
